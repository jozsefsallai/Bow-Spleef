...
public class Metrics {
    ...
    public boolean start()
    {
        synchronized (this.optOutLock)
        {
            ...
            this.task = this.plugin.getServer().getScheduler().runTaskTimerAsynchronously(BowSpleef.getInstance(), new Runnable()
            {
    ...
    public boolean isOptOut()
    {
        synchronized (this.optOutLock)
        {
            try {
                ...
                if (this.debug) {
                    Bukkit.getLogger().log(Level.INFO, new StringBuilder().append("[Metrics] ").append(ex.getMessage()).toString());
                }
                return true;
            ...
            throws IOException
    {
        ...
        if (this.debug) {
            System.out.println(new StringBuilder().append("[Metrics] Prepared request for ").append(pluginName).append(" uncompressed=").append(uncompressed.length).append(" compressed=").append(compressed.length).toString());
        }
    ...
    private static String escapeJSON(String text)
    {
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            switch (chr) {
                ...
                default:
                    if (chr < ' ') {
                        String t = new StringBuilder().append("000").append(Integer.toHexString(chr)).toString();
                        builder.append(new StringBuilder().append("\\u").append(t.substring(t.length() - 4)).toString());
                    } else {
                        builder.append(chr);
                    }
                    break;
